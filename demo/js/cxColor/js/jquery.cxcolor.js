...
(function($){
	$.fn.cxColor=function(settings){
		...
		theColorPanel.fn.show=function(pane_top, pane_left){
			...
			if(!pane_top) pane_top=theColorPanel.jqObj.offset().top;
			if(!pane_left) pane_left=theColorPanel.jqObj.offset().left;
		...
		theColorPanel.fn.change=function(c){
			color_now=c;
			if(c!=='') {
				theColorPanel.jqObj.val(color_now).attr("class", "input_cxcolor");
				theColorPanel.jqObj.val(color_now).css("backgroundColor", color_now);
			} else {
				theColorPanel.jqObj.val(color_now).attr("class", "input_nocolor");
				theColorPanel.jqObj.val(color_now).css("backgroundColor", "#fff");
			}
		...
		//清除颜色组件
		theColorPanel.fn.remove = function(){
			theColorPanel.jqObj.trigger("remove");
			color_pane.remove();
			lock_bg.remove();
			delete theColorPanel;
		};
		...
		reportColorPanel={
			...
			remove:theColorPanel.fn.remove
		}
